...
func TestMiddleware_RejectsMissingAndBadTokens(t *testing.T) {
	...
	h := Middleware([]byte("secret"))(http.HandlerFunc(func(_ http.ResponseWriter, _ *http.Request) {
		t.Error("handler should not be called")
	}))

	// Missing.
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequestWithContext(context.Background(), http.MethodGet, "/ws", nil))
...
func TestMiddleware_AcceptsTokenViaQueryParam(t *testing.T) {
	...
	tok, _, err := iss.Sign("device-q")
	if err != nil {
		t.Fatalf("sign: %v", err)
	}

	h := Middleware([]byte("secret"))(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))

	req := httptest.NewRequestWithContext(context.Background(), http.MethodGet, "/ws?token="+tok, nil)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
